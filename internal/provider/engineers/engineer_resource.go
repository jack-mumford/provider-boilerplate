...
func (r *EngineerResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	engineers, err := r.client.GetEngineer(state.ID.ValueString())
	...
	state.ID = types.StringValue(engineers.ID)
	state.Name = types.StringValue(engineers.Name)
	state.Email = types.StringValue(engineers.Email)
...
func (r *EngineerResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Fetch updated engineer after update
